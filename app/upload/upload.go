...
type UpCmd struct {
	...
	SessionTag        bool
	TagViaSidecar     bool
	ChecksumCachePath string // Path to persist known checksums (uploaded or on server); opt-in
...
func (uc *UpCmd) RegisterFlags(flags *pflag.FlagSet) {
	...
	flags.StringVar(&uc.ChecksumCachePath, "checksum-cache", "", "Path to a file containing checksums already known (uploaded or on server); if set, the set is preloaded and updated on exit")
